...
final class APIService: NSObject, URLSessionDelegate, URLSessionTaskDelegate {
  ...
  static func baseURL(from sessionId: String) -> String {
    if sessionId.hasPrefix("vs_live_") {
      return "https://api.kayle.id"
    } else if sessionId.hasPrefix("vs_test_") {
      // HTTP is safe over Tailscale (traffic is encrypted at network layer)
